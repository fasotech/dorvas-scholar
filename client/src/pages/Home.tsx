...
function CreatePanel({ label, onClose }: { label: string; onClose: () => void }) {
  return <div className="create-overlay" role="dialog" aria-modal="true" aria-label={label}><div className="create-panel"><button className="icon-button close" onClick={onClose} aria-label="Close panel"><X size={19} /></button><span className="stamp"><CircleDot size={14} /> Protected action</span><h2>{label}</h2><p>Live write procedures will be enabled only after an administrator has linked your identity and Atlas network access is confirmed. This interface never saves browser-only demo records.</p><div className="panel-actions"><Button variant="outline" onClick={onClose}>Close</Button></div></div></div>;
}
