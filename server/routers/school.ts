...
export const schoolRouter = router({
  dashboard: publicProcedure.query(async ({ ctx }) => {
    ...
    return await getDashboard({ openId: user.id || user.openId, email: user.email, name: user.name });
  }),
  records: publicProcedure
    .input(z.object({ section: z.enum(dashboardSections as any), query: z.string().optional().default("") }))
    .query(async ({ ctx, input }) => {
      const user = ctx.user || { id: "local-dev", name: "Dev User", email: "dev@example.com" };
      return await getRecords({ openId: user.id || user.openId, email: user.email, name: user.name }, input.section, input.query);
    })
});
