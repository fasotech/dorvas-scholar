...
import { CBTExam, CBTQuestion, ClassNote } from "./server/models/school";
...
async function run() {
  ...
  await CBTQuestion.create([
    ...
    {
      examId: exam._id,
      questionText: "Simplify: (3^2) * (3^3)",
      options: ["3^5", "3^6", "9^5", "9^6"],
      correctOptionIndex: 0,
      marks: 1
    }
  ]);

  // Seed a sample official Teacher Assessment
  ...
  console.log("Seeded CBT and Notes successfully.");
  await disconnect();
}
run();
